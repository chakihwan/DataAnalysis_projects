...
EarlyStopping 추가 후 DNN의 Task A Recall이 0.667 → 0.750으로 향상되어 XGBoost와 동률이 됐다. 현재 threshold=0.5 기준으로는 두 모델 Recall이 동일하지만 DNN의 AUC(0.950)가 XGBoost(0.779)보다 크게 높아 확률 분포 분리가 더 우수하다.

**Threshold 최적화 적용 시 DNN 우위가 뚜렷해진다 (섹션 9 참조):**
- DNN threshold=0.40: Recall=**0.917**, F1=0.846 (불량 12개 중 11개 탐지)
- XGBoost threshold=0.48: Recall=0.833, F1=0.800

불량 유출을 최소화해야 하는 현장에는 **DNN + threshold=0.40**이 최선이다.
...
| `models/dnn_task_a.pt` | ~수백 KB | Task A DNN — 불량 유출 최소화 시나리오에서 threshold=0.40으로 운영 권고. scaler 파라미터 내장 |
| `models/dnn_task_b.pt` | ~수백 KB | Task B DNN (GPU 재학습 후 AUC 0.767로 개선. XGBoost 대비 전 지표 우위) |
